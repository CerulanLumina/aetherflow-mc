...
class AetherPumpRenderer(dispatcher: BlockEntityRenderDispatcher) :
    BlockEntityRenderer<AetherPump>(dispatcher) {
    ...
    override fun render(
        ...
        if (!aetherPump.cachedState[BlockAetherPump.Props.VALID]) return

        val time = aetherPump.world!!.time
        matrixStack.push()

//        for (kp in 0..1000)


        val attach = aetherPump.cachedState[BlockAetherPump.Props.ATTACHED]!!
    ...
    override fun rendersOutsideBoundingBox(af: AetherPump): Boolean = true
}